...
int main(int argc, char *argv[]){
	...
	if(block){
		while((size = readin(buffer, stdin, newline))){
			temptr = warpaction(argv[0], decode, encode, buffer, temptr);
			writeres(argv[0], outfile, size, temptr, newline);
		}
	}
	if(infile != NULL){
		FILE *fp = fopen(infile, "r");
		while((size = readin(buffer, fp, newline))){
			temptr = warpaction(argv[0], decode, encode, buffer, temptr);
			writeres(argv[0], outfile, size, temptr, newline);
		}
		fclose(fp);
	...
	while(optind < argc){
		while((size = base64_overlen(argv[optind], 0))){
			warpaction(argv[0], decode, encode, argv[optind++], buffer);
			writeres(argv[0], outfile, size, buffer, newline);
		}
	}
